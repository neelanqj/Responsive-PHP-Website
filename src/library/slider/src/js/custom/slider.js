 ...
 * Date: 2013-09-14
 *
 * Description: This is the code for the configurable slider.
 * 
 */
...
function Slider(container, nav) {
	...
	$('#slider-nav').width(_self.imgWidth), 
	_self.current = 0;
	
	// Add the button images
	$('#slider-nav').append("<img class='tilt' id='sliderImg-0' src='src/library/slider/src/img/slidernav_current.gif' width='33px' height='33px'>");
	
	for(i=0; i < _self.imgsLen - 1; i++) {
		$('#slider-nav').append("<img class='tilt' id='sliderImg-"+ (i+1) +"' src='src/library/slider/src/img/slidernav.gif' width='33px' height='33px'>");
	}
	
	// Initialize the buttons
	_self.nav.find('img').on('click', function() {
        	_self.setFrame($(this).attr("id"));
            _self.transition();
            });
};
...
Slider.prototype.transition = function ( frame ) {
	...
	_self.container.animate({
		'margin-left': -( frame * _self.imgWidth ) || -( _self.current * _self.imgWidth )
	});
};
...
Slider.prototype.setCurrent = function ( dir ) {
	...
	$('#sliderImg-'+pos).attr('src', 'src/library/slider/src/img/slidernav.gif');
	pos += ( ~~( dir === 'next' ) || -1 );
	new_pos = (pos < 0) ? _self.imgsLen - 1 : pos % _self.imgsLen; 
	
	$('#sliderImg-' + new_pos).attr('src', 'src/library/slider/src/img/slidernav_current.gif');
...
Slider.prototype.setFrame = function ( imgID ) {
	...
	$('#sliderImg-'+pos).attr('src', 'src/library/slider/src/img/slidernav_current.gif');
};
